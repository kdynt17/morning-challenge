...
import { get, getDatabase, limitToLast, onValue, orderByChild, push, query, ref, serverTimestamp, set, update } from "https://www.gstatic.com/firebasejs/10.12.5/firebase-database.js";
...
const openSignupButton = $("#open-signup-button");
const closeSignupButton = $("#close-signup-button");
...
const pageNames = { home: "오늘의 문제", leaderboard: "포인트 순위", auction: "포인트 경매장", profile: "내 정보", admin: "선생님 관리" };

openSignupButton.addEventListener("click", () => { signupDialog.showModal(); setMessage(signupMessage, "가입하면 자동으로 로그인됩니다."); });
closeSignupButton.addEventListener("click", () => signupDialog.close());
...
navButtons.forEach((button) => button.addEventListener("click", () => showPage(button.dataset.targetPage)));
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user) {
    ...
    setMessage(authMessage, "선생님 계정은 admin@admin으로 로그인하세요.");
    return;
  }
...
async function handleLogin(event) {
  ...
  const email = rawLogin === ADMIN_LOGIN ? ADMIN_AUTH_EMAIL : rawLogin;
  try {
...
async function handleSignup(event) {
  ...
  try {
    ...
    await set(ref(db, `users/${credential.user.uid}`), { name, email, role: "student", points: 0, todayResult: "-", createdAt: serverTimestamp() });
    signupDialog.close();
  } catch (error) {
...
async function ensureProfile(user) {
  ...
  const profile = { name: "학생", email: user.email, authEmail: user.email, role: "student", points: 0, todayResult: "-", createdAt: serverTimestamp() };
...
function showPage(page) {
  document.querySelectorAll(".page").forEach((section) => section.classList.toggle("active-page", section.dataset.page === page));
  navButtons.forEach((button) => button.classList.toggle("active", button.dataset.targetPage === page));
...
function subscribeChallenge() {
  ...
  const unsubscribe = onValue(ref(db, `dailyChallenges/${today}`), (snapshot) => {
    if (!snapshot.exists()) {
...
function subscribeLeaderboard() {
  ...
  const unsubscribe = onValue(studentsQuery, (snapshot) => {
    if (!snapshot.exists()) { leaderboardList.innerHTML = `<p class="empty-text">아직 포인트 기록이 없습니다.</p>`; return; }
    const students = [];
    snapshot.forEach((child) => { const student = child.val(); if (student.role !== "teacher") students.push(student); });
    students.sort((a, b) => Number(b.points ?? 0) - Number(a.points ?? 0));
    leaderboardList.innerHTML = students.length ? students.map(renderRankItem).join("") : `<p class="empty-text">아직 학생 기록이 없습니다.</p>`;
  });
  unsubscribers.push(unsubscribe);
}

function renderRankItem(student, index) {
  return `<article class="rank-item"><span class="rank-number">${index + 1}</span><div><p class="rank-name">${escapeHtml(student.name ?? student.email ?? "학생")}</p><p class="rank-meta">${escapeHtml(student.todayResult ?? "-")}</p></div><span class="rank-points">${Number(student.points ?? 0)}점</span></article>`;
}
...
function subscribeAuctions() {
  ...
  const unsubscribe = onValue(auctionsQuery, (snapshot) => {
    if (!snapshot.exists()) { auctionList.innerHTML = `<p class="empty-text">등록된 상품이 없습니다.</p>`; return; }
    const auctions = [];
    snapshot.forEach((child) => { const item = child.val(); if (item.visible) auctions.push({ id: child.key, ...item }); });
    if (!auctions.length) { auctionList.innerHTML = `<p class="empty-text">표시할 상품이 없습니다.</p>`; return; }
    auctions.sort((a, b) => Number(a.endsAt ?? 0) - Number(b.endsAt ?? 0));
    auctionList.innerHTML = auctions.map(renderAuctionItem).join("");
    auctionList.querySelectorAll("button[data-auction-id]").forEach((button) => button.addEventListener("click", () => handleBid(button.dataset.auctionId)));
  });
  unsubscribers.push(unsubscribe);
}
...
function renderAuctionItem(item, index) {
  ...
  return `<article class="auction-item"><div class="auction-visual ${colorClass}">${escapeHtml(item.category ?? "상품")}</div><div class="auction-body"><p class="eyebrow">${status}</p><h3>${escapeHtml(item.title ?? "상품")}</h3><p>현재가 ${currentPrice}점 · ${formatDeadline(item.endsAt)}</p><button type="button" data-auction-id="${item.id}" ${item.status === "open" ? "" : "disabled"}>입찰하기</button></div></article>`;
}

async function handleAnswerSubmit(event) {
  event.preventDefault();
  if (!currentUser || !currentChallengeId) { setMessage(answerMessage, "로그인 후 오늘의 문제를 불러와 주세요."); return; }
  const answer = answerInput.value.trim();
  if (!answer) return;
  await set(push(ref(db, "submissions")), { challengeId: currentChallengeId, uid: currentUser.uid, answer, status: "pending", createdAt: serverTimestamp() });
  answerInput.value = "";
...
async function handleBid(auctionId) {
  ...
  await set(push(ref(db, "bids")), { auctionId, uid: currentUser.uid, amount, createdAt: serverTimestamp() });
}
...
async function handleChallengeSave(event) {
  ...
  const keywords = $("#admin-answer-keywords").value.split(",").map((value) => value.trim()).filter(Boolean);
  await update(ref(db), {
    [`dailyChallenges/${dateKey}`]: { title, description, status: "open", createdBy: currentUser.uid, updatedAt: serverTimestamp() },
    [`challengeAnswers/${dateKey}`]: { keywords, exactAnswers: [], updatedAt: serverTimestamp() },
  });
  challengeForm.reset();
  $("#admin-challenge-date").value = getKoreaDateKey();
...
async function handleAuctionSave(event) {
  ...
  await set(push(ref(db, "auctions")), {
    title: $("#admin-auction-title").value.trim(), category: $("#admin-auction-category").value.trim(), visible: true, status: "open", startPrice, currentPrice: startPrice, endsAt, createdBy: currentUser.uid, createdAt: serverTimestamp(),
  });
  auctionForm.reset();
...
function formatDeadline(timestamp) { if (!timestamp) return "마감 시간 미정"; return new Date(Number(timestamp)).toLocaleString("ko-KR", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }); }
...
function clearSubscriptions() { unsubscribers.forEach((unsubscribe) => unsubscribe()); unsubscribers = []; }
